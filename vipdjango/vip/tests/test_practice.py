...
def selection(status="addressed", question="", dialogue="That is difficult to take in.", **extra):
    return {"answer_status": status, "question_id": question, "dialogue": dialogue,
            "addressed_question_ids": [], "ready_to_close": False, "readiness_evidence": "", **extra}


def scripted_reply(scenario, messages, pending, available, progress):
    question = available[0] if available else None
    return selection(question=question["id"] if question else "",
                     dialogue=question["text"] if question else f"I need a moment to think about what you said at this point ({progress['turn']}).")
...
class CorePracticeTests(SimpleTestCase):
    ...
    def test_nurse_question_is_answered_without_losing_pending_concern(self):
        ...
        text, complete, state = core_questions.respond(scenario, [message("student", "What do you understand?")], progress,
    ...
    def test_invalid_dialogue_or_ids_fail_without_mutating_committed_state(self):
        ...
        for result in ({}, selection(question="invented"), selection(dialogue="I am your nurse. Give her 100 mg."),
    ...
    def test_unclear_answer_allows_one_specific_followup(self):
        scenario = parse_scenario_prompt(scenario_text())
        progress = core_questions.opening_state(scenario)
        _, complete, state = core_questions.respond(scenario, [], progress,
            lambda *args: selection("unclear", question="opening", dialogue="Could improvement mean recognizing us?"))
        self.assertFalse(complete)
        self.assertEqual(state["core_question_state"]["pending"]["id"], "opening")
    ...
    def test_stop_works_before_opening_without_api(self):
        ...
        with patch.object(engine, "_assess_core_reply") as model:
            _, complete, state = engine.respond(scenario_text(), [message("student", "Please stop.")])
    ...
    def test_twenty_fifth_input_gets_response_then_closes(self):
        engine = ConversationEngine("", "test")
        with patch.object(engine, "_assess_core_reply", return_value=selection(dialogue="I understand that you will stay with us. What happens next?")) as model:
            text, complete, state = engine.respond(scenario_text(), [message("student", "I will stay with you.")] * MAX_TURNS)
    ...
    def test_duplicate_dialogue_has_one_repair_before_commit(self):
        scenario = parse_scenario_prompt(scenario_text())
        assess = MagicMock(side_effect=[selection("learner_question", dialogue="That is hard to hear."), selection("learner_question", dialogue="I need a moment to take that in.")])
        messages = [message("assistant", "That is hard to hear."), message("student", "Take your time.")]
    ...
    def test_invalid_or_duplicate_final_dialogue_still_ends_safely(self):
        scenario = parse_scenario_prompt(scenario_text())
        messages = [message("student", "Please take your time.")] * MAX_TURNS + [message("assistant", "I need some time.")]
        for text in ("I need some time.", "I am your nurse. Give her 100 mg."):
            output, complete, info = core_questions.respond(scenario, messages, None,
    ...
    def test_legacy_cap_and_completed_session_do_not_call_model(self):
        engine = ConversationEngine("", "test")
        _, complete, state = engine.respond("## Role\nYou are a worried patient.", [message("student", "hello")] * MAX_TURNS)
        self.assertTrue(complete)
        self.assertEqual(state["reason"], "turn_limit")
    ...
    def test_later_repair_clears_earlier_unresolved_concern(self):
        ...
        _, _, state = core_questions.respond(scenario, [], progress, lambda *args: selection(question=args[3][0]["id"], dialogue=args[3][0]["text"]))
    ...
    def test_pacing_moves_to_each_theme_even_with_repeated_nurse_questions(self):
        ...
        for turn, theme in [(8, 1), (15, 2)]:
            prepared = core_questions.prepare(scenario, [message("student", "What worries you?")] * turn, progress)
            self.assertEqual(prepared["active_theme"], theme)
            self.assertFalse(prepared["must_advance"])
            self.assertTrue(all(q["theme"] == theme for q in core_questions.candidates(scenario, prepared)))
    ...
    def test_nurse_led_transition_can_use_adjacent_theme_without_reopening_old_one(self):
        ...
        def reply(scenario, messages, pending, available, state):
            ...
            return selection("partial", question=candidate["id"], dialogue="But is the feeding causing her discomfort now?")
        ...
        self.assertEqual(core_questions.prepare(scenario, nurse, result["core_question_state"])["active_theme"], 1)
    ...
    def test_reasonable_explanation_cannot_trigger_another_confirmation(self):
        ...
        _, _, info = core_questions.respond(scenario, [message("student", "I would expect so, yes.")], progress, assess)
    ...
    def test_reflection_only_reply_is_rewritten_to_advance(self):
        ...
        def assess(scenario, messages, pending, available, state):
            ...
            return selection(question=available[0]["id"], dialogue=available[0]["text"])
        text, _, info = core_questions.respond(scenario, [message("student", "Months to years.")], None, assess)
    ...
    def test_nurse_question_does_not_consume_a_repair_but_second_probe_is_blocked(self):
        scenario = parse_scenario_prompt(scenario_text())
        progress = core_questions.opening_state(scenario)
        _, _, result = core_questions.respond(scenario, [message("student", "What are you hoping for?")], progress,
            lambda *args: selection("learner_question", dialogue="I hope she can recognize us again."))
        ...
        with self.assertRaises(ValueError):
            core_questions.respond(scenario, [], progress,
                lambda *args: selection("unsafe", question="opening", dialogue="How can you know that?"))

    def test_spoken_new_question_matches_selected_id_without_repeating_old_question(self):
        scenario = parse_scenario_prompt(scenario_text())
        progress = core_questions.opening_state(scenario)
        def assess(scenario, messages, pending, available, state):
            return selection(question=available[0]["id"], dialogue="But have specialists actually seen her? I understand your answer.")
        text, _, info = core_questions.respond(scenario, [message("student", "I would expect so, yes.")] * 9, progress, assess)
        self.assertEqual(text, info["core_question_state"]["pending"]["text"])
    ...
    def test_new_question_keeps_the_answer_when_nurse_asks_about_hopes(self):
        ...
        self.assertEqual(core_questions.render_question(answer, question, "learner_question", 8), answer + " " + question["text"])
        self.assertEqual(core_questions.render_question(answer, question, "addressed", 8), question["text"])
    ...
    def test_standalone_question_needs_no_generated_filler(self):
        ...
        text, _, info = core_questions.respond(scenario, [message("student", "Yes.")], None, model)
    ...
    def test_final_pause_is_not_appended_twice(self):
        ...
        text, complete, _ = core_questions.respond(scenario, [message("student", "I am here.")] * MAX_TURNS, None,
    ...
    def test_success_requires_coverage_repair_and_readiness_check(self):
        ...
        ready = selection(ready_to_close=True, readiness_evidence="Do you feel ready to begin?")
        _, complete, _ = core_questions.respond(scenario, nurse, progress, lambda *args: ready)
        self.assertFalse(complete)
        ready["addressed_question_ids"] = [q["id"] for q in core_questions.question_bank(scenario)]
        text, complete, state = core_questions.respond(scenario, nurse, progress, lambda *args: ready)
        self.assertTrue(complete)
        self.assertEqual(text, scenario.closing)
        self.assertTrue(state["ending_ready"])
        ready["readiness_evidence"] = "invented check"
        self.assertFalse(core_questions.respond(scenario, nurse, progress, lambda *args: ready)[1])

    def test_first_turn_can_answer_understanding_question(self):
        engine = ConversationEngine("", "test")
        answer = "Daniel told me Mom would not want a feeding tube in this condition. I have not read the whole document."
        with patch.object(engine, "_assess_core_reply", return_value=selection("learner_question", dialogue=answer)):
            text, complete, _ = engine.respond(scenario_text(2), [message("student", "Hello Rachel. What did Daniel tell you?")])
        self.assertEqual(text, answer)
        self.assertFalse(complete)

    def test_model_request_includes_full_conditional_guidance_and_pacing(self):
        engine = ConversationEngine("", "test")
        client = MagicMock()
        ...
        with patch.object(engine, "_openai_client", return_value=client):
            engine._assess_core_reply(scenario, [message("student", "What do you know?")], None, [], {"turn": 2})
        request = client.responses.create.call_args.kwargs
        context = request["input"][0]["content"]
        self.assertFalse(request["store"])
        self.assertEqual(request["input"][-1]["role"], "user")
        for fragment in ("What Rachel knows before the nurse arrives", "How Rachel responds", "learner_question", "25 learner", "readiness"):
...
class ChatWorkflowTests(TestCase):
    ...
    def test_instructor_test_chat_lists_active_prompts_only(self):
        self.user.groups.add(Group.objects.create(name="Professor"))
        self.prompt.is_active = False
        self.prompt.save()
        active_prompt = RolePrompt.objects.create(title="Active Rachel", content=scenario_text(2), is_active=True)
        response = self.client.get(reverse("vip:professor_test_chat"), {"prompt": self.prompt.id})
        self.assertEqual(response.context["selected_prompt"], active_prompt)
        self.assertEqual(list(response.context["active_prompts"]), [active_prompt])
        self.assertContains(response, active_prompt.title)
    ...
    def test_feedback_import_creates_distinct_repeatable_drafts(self):
        ...
        self.assertTrue(all("Hidden" not in draft.title and "Meta Instructions" in draft.content for draft in drafts))
    ...
    def test_followup_revision_import_preserves_previous_revision(self):
        ...
        self.assertEqual(RolePrompt.objects.filter(title__contains="September 17, 2026").count(), 2)
        previous.refresh_from_db()
        self.assertEqual(previous.content, "Faculty edits")

    def test_prompt_editor_preserves_full_hidden_reactions(self):
        from vip.forms import RolePromptForm
        data = RolePromptForm.initial_from_content(scenario_text(2), title="Revised hospice")
        form = RolePromptForm(data)
        self.assertTrue(form.is_valid(), form.errors)
        # The editor folds legacy Meta Instructions into background context.
        self.assertIn("What Rachel knows before the nurse arrives", form.cleaned_data["background_context"])
        self.assertIn("If the nurse says", form.cleaned_data["background_context"])
